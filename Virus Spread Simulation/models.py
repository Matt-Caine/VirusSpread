...
def SIRD(sbx_healthy, sbx_infected, sbx_days, sbx_propagation, sbx_r_chance,sbx_mortality):
    ...
    axs[0, 0].plot(t, I, 'r', linestyle='--', label='Infected', color='tab:orange')
    axs[0, 0].plot(t, R, 'g', label='Recovered & protected', color='tab:green')
    axs[0, 0].plot(t, D, 'c', label='Dead', color='crimson')
    ...
    axs[0, 0].legend(['Susceptible', 'Infected', 'Recovered & Protected','Dead'], loc='lower center',
                     ncol=1, fancybox=True, bbox_to_anchor=(0.87, 0.70))

    # TOP Right plot---------------------------------------------------------

    labels = 'Susceptible', 'Infected', 'Recovered & protected', 'Dead'
    colors = ['tab:blue', 'tab:orange', 'tab:green','crimson']

    sizes = [S[-1], I[-1], R[-1], D[-1]]
